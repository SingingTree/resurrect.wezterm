...
local function shell_mkdir(path)
	if utils.is_windows then
		...
		return os.execute("mkdir " .. quoted .. " >/dev/null 2>&1")
	end
end
...
local function mkdir_if_missing(path)
	...
	-- shell_mkdir can fail even when the directory already exists: on Windows,
	-- os.rename above can give a false negative when another process (e.g.
	-- WezTerm itself) holds an open handle to the path. Fall back to a probe
	-- write, which is reliable regardless of handle state.
